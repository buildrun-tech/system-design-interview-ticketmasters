...
```bash
# Gerar chave privada EC P-256 em formato SEC1
openssl ecparam -name prime256v1 -genkey -noout -out ec_private_sec1.pem

# Converter para PKCS#8 (OBRIGATÓRIO — SmallRye JWT exige PKCS#8, não SEC1)
openssl pkcs8 -topk8 -nocrypt -in ec_private_sec1.pem -out ecPrivateKey.pem

# Extrair chave pública
openssl ec -in ec_private_sec1.pem -pubout -out ecPublicKey.pem
```

> **Atenção:** `openssl ecparam` gera chave no formato SEC1 (`BEGIN EC PRIVATE KEY`). O SmallRye JWT só aceita PKCS#8 (`BEGIN PRIVATE KEY`). Sem a conversão com `pkcs8 -topk8`, a assinatura falha com `SRJWT05028`.
...
Properties necessárias no `application.properties` para ES256:

```properties
# Verificação (verifier)
mp.jwt.verify.publickey.location=classpath:ecPublicKey.pem
smallrye.jwt.verify.algorithm=ES256          # propriedade SmallRye, NÃO mp.jwt.verify.algorithm

# Assinatura (signer)
smallrye.jwt.sign.key.location=classpath:ecPrivateKey.pem
smallrye.jwt.new-token.signature-algorithm=ES256  # propriedade SmallRye, NÃO smallrye.jwt.sign.algorithm
```
