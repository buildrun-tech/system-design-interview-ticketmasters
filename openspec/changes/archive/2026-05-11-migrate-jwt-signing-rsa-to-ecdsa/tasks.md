...
- [x] 3.1 Executar os testes de integração existentes (`CreateUserIT`, `CreateBookingIT`) para garantir que o fluxo de autenticação continua funcional
- [x] 3.2 Verificar manualmente com `curl` que o token retornado por `/auth/token` possui `"alg": "ES256"` no header (decodificar com `base64`)
- [x] 3.3 Verificar que um endpoint protegido aceita o token ES256 emitido
- [x] 3.4 Confirmar que claims (`iss`, `sub`, `upn`, `groups`, `email`/`app_name`, `exp`) estão preservados no token
