...
- [ ] 6.2 Executar `terraform plan` e verificar que não há mudanças destrutivas inesperadas além do NLB SG e da remoção do VPC Link v1
- [ ] 6.3 Executar `terraform apply` e confirmar que todos os recursos são criados com sucesso
- [ ] 6.4 Testar o endpoint do API Gateway via `curl` ou Postman: GET, POST, verificar headers e query params chegando corretamente no ECS
